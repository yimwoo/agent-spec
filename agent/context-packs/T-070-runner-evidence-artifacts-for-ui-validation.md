# T-070: Runner evidence artifacts for UI validation

Type: `implementation`
Originating DCR: `DCR-0040`

## Goal

Runner evidence artifacts for UI validation

## Requirements

- `R-175` Runner results support structured UI validation evidence artifacts (P1, medium)
...
- `docs/change-requests/DCR-0040-runner-evidence-artifacts-for-ui-validation.md`
...
| `agentspec/runner.py` | confirmed; code target |
| `agent/context-packs/T-070-runner-evidence-artifacts-for-ui-validation.md` | inferred; support artifact |
| `docs/change-requests/DCR-0040-runner-evidence-artifacts-for-ui-validation.md` | confirmed; support artifact |
| `docs/traceability/requirements.yml` | confirmed; support artifact |
| `tests/test_runner_package.py` | confirmed; task verification |
...
- Runner packages include an evidence result template with UI-relevant artifact kinds such as screenshot, DOM snapshot, navigation trace, console log, network log, video, and trace.
- aspec run result accepts valid evidence and records it in the executor event.
- Invalid evidence is rejected before run state changes.
- Existing runner results without evidence remain valid.
- Tests cover package template shape, valid evidence threading, and invalid evidence rejection.
